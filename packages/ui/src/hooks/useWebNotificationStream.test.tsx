...
test('shares one control stream, deduplicates main-stream delivery, and retires old runtimes', async () => {
  ...
  try {
    ...
    const payload = { title: 'Done', body: 'Ready', sessionId: 'session-one', kind: 'complete', directory: '/repo', variant: 'success' as const };
    await act(async () => {
      source.emit(payload);
    ...
    expect(successToasts).toEqual([{ title: 'Done', id: 'success|Done|Ready', description: 'Ready' }]);
